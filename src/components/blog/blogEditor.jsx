...
class BlogEditor extends Component {
  ...
  getThumbnailPicture = () => {
    ...
    if (thumbnailPicture !== null) {
      if (
        typeof thumbnailPicture.url !== "undefined" &&
        typeof thumbnailPicture.reference !== "undefined"
      ) {
        return (
          <img
            src={thumbnailPicture.url}
            alt={thumbnailPicture.reference}
            className="img-fluid"
            style={{ width: "100%" }}
          />
        );
  ...
  handleImageChange = async (event) => {
    if (event.target.files && event.target.files[0]) {
      ...
      this.setState({
        thumbnailPicture: event.target.files[0],
  ...
  handleSubmitBtnClicked = async () => {
    ...
    if (isValid === true) {
      ...
      while ((result = regex.exec(messageToSend)) !== null) {
        ...
        if (
          dataURI.indexOf("data:image/") !== -1 &&
          dataURI.indexOf("https://firebasestorage.googleapis.com/") === -1
        ) {
          await fetch(dataURI).then(async (res) => {
            await res.blob().then(async (bloblFile) => {
              const newFile = new File([bloblFile], "news", {
                type: "image/png",
              });
              try {
      ...
      messageToSend = messageToSend.replace(/<img (.*?)>/g, (imageTag) => {
        ...
        if (src.indexOf("data:image/") !== -1) {
          imageTag = imageTag.replace(
            regexForSrc,
            `src="${files[counter].url}" alt="${files[counter].reference}" class="img-fluid"`
          );
        }
        counter++;
        return imageTag;
      ...
      if (typeof thumbnailPicture.url === "undefined") {
        ...
        thumbnailPictureToSend = await BlogService.uploadPicture(
          compressedImage
        );
      } else {
      ...
      if (action === "update") {
        if (typeof thumbnailPicture.reference === "undefined") {
          await BlogService.deleteImage(
            this.state.originalBlog.thumbnail.reference
          );
        }
      }
      if (action === "update") {
        ...
        while ((result = regex.exec(previousMessage)) !== null) {
          ...
          if (imageRef !== null) {
            ...
            if (
              imageURL.indexOf("firebasestorage.googleapis.com") !== -1 &&
              messageToSend.indexOf(reference) === -1
            ) {
      ...
      <React.Fragment>
        <div className="card">
          ...
          <div className="card-body">
            <div className="row form-group">
              ...
              <div className="col-5">
                ...
                />
                {formSubmitted === true &&
                  typeof validations.author !== "undefined" && (
            ...
            <div className="row form-group">
              ...
              <div className="col-5">
                {thumbnailPicture !== null && this.getThumbnailPicture()}
                {(action === "create" || action === "update") && (
                  <React.Fragment>
                    ...
                    />
                    {formSubmitted === true &&
                      typeof validations.thumbnail !== "undefined" && (
          ...
          <div className="card-footer">
            ...
            <Link className="btn btn-secondary ml-2" to="/blog">
              Cancel
            </Link>
          </div>
        </div>
