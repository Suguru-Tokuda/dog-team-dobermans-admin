...
class PuppyMessageEditor extends Component {
  ...
  handleUpdateBtnClicked = async () => {
      ...
      if (
        ...
        await fetch(dataURI).then(async (res) => {
          await res.blob().then(async (bloblFile) => {
            const newFile = new File([bloblFile], "puppyUnavailableMessage", {
              type: "image/png",
            });
            try {
              const options = {
              ...
              const image = await HomepageContentService.uploadPicture(
                compressedFile,
                "puppy-unavailable-message"
              );
              files.push(image);
            } catch (err) {
              toastr.error(err);
    ...
    let counter = 0;
    messageBodyToSend = messageBodyToSend.replace(
      /<img (.*?)>/g,
      (imageTag) => {
        const src = regexForSrc.exec(imageTag)[1];
        if (
          src.indexOf("data:image/") !== -1 &&
          src.indexOf("https://firebasestorage.googleapis.com/")
        ) {
    ...
    while ((result = regex.exec(originalBody)) !== null) {
      ...
      const refRegex =
        /hompageContents%2Fpuppy-unavailable-message%2F(.*?)\?alt/g;
      const imageRef = refRegex.exec(imageURL);
      if (imageRef !== null) {
        const reference = imageRef[1];
        if (
          imageURL.indexOf("firebasestorage.googleapis.com") !== -1 &&
          messageBodyToSend.indexOf(reference) === -1
        ) {
          await HomepageContentService.deleteFile(
            `hompageContents/puppy-unavailable-message/${reference}`
          );
        }
      }
    }
    ...
    return (
      <div className="card">
        ...
        <div className="card-footer">
          ...
          <Link type="button" className="btn btn-secondary ml-2" to="/puppies">
            Cancel
          </Link>
        </div>
      </div>
